...
class _ViewTicketsScreenState extends State<ViewTicketsScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            const Text('Mis Tickets', style: TextStyle(fontSize: 22, color: Colors.white)),
  ...
  Widget _buildEmptyState() => Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          ElevatedButton(onPressed: () => _navigateToCreateTicket(context), child: const Text('Crear primer ticket')),
        ]),
      );

  Widget _buildTicketsList(List<Ticket> tickets) => ListView.builder(
            ...
            child: ListTile(
              ...
              leading: (ticket.prioridad != null && ticket.prioridad!.isNotEmpty)
                  ? Container(
                      width: 16,
                      height: 16,
                      decoration: BoxDecoration(color: _getPriorityColor(ticket.prioridad!), shape: BoxShape.circle),
                    )
                  : const Icon(Icons.priority_high, color: Colors.grey),
              title: Text(ticket.titulo, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
  ...
  void _navigateToTicketDetail(BuildContext context, Ticket ticket) {
    // Aquí puedes implementar la pantalla detalle si lo deseas
  }
  ...
  Future<void> _generatePdf(Ticket ticket) async {
      ...
      pw.Page(
        ...
        build: (context) {
          return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text('Ticket de soporte', style: pw.TextStyle(fontSize: 26, fontWeight: pw.FontWeight.bold)),
            ...
            pw.Container(
              ...
              child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                ...
                pw.Text(ticket.titulo, style: const pw.TextStyle(fontSize: 16)),
                pw.SizedBox(height: 10),
                pw.Text('Descripción:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                pw.Text(ticket.descripcion, style: const pw.TextStyle(fontSize: 14)),
                pw.SizedBox(height: 10),
                pw.Text('Fecha de creación:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                ...
                pw.SizedBox(height: 10),
                if (ticket.prioridad != null && ticket.prioridad!.isNotEmpty) ...[
                  pw.Text('Prioridad:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
    ...
    try {
      ...
      final safeTitle = ticket.titulo.replaceAll(RegExp(r'[^\w\s]+'), '');
      final fileName = 'ticket_${safeTitle}_${DateTime.now().millisecondsSinceEpoch}.pdf';
